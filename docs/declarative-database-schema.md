...
With the traditional [setup resources](maho-for-devs/setup-resources.md) system, every schema change means bumping a module version and writing a new `upgrade-X.Y.Z-A.B.C.php` with the exact `ALTER` statements. Over time this becomes a long, hard-to-review history of incremental changes, and the current shape of a table can only be reconstructed by reading every script in order.
...
The declarative schema replaces only the *structure* part of [setup resources](maho-for-devs/setup-resources.md). Data work stays imperative, split by when it needs to run:
