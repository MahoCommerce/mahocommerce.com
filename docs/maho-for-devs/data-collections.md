...
Magento doesn't work that way. Instead, the concept of [Lazy Loading](https://en.wikipedia.org/wiki/Lazy_loading) is used. In simplified terms, Lazy loading means that no SQL calls are made until the client-programmer needs to access the data. That means when you do something something like this
...
Magento has a number of these english language like filters that will bring a [tear of remembrance](http://www.shlomifish.org/lecture/Perl/Newbies/lecture1/conditionals/string.html) (and perhaps pain) to any old perl developers in the audience.
