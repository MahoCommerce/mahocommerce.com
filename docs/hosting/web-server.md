...
- Deny access to the file types that disclose the installation: `.txt`, `.json`, `.dist`, `.flag`,
  `.ip`, `.lock`, `.neon`, `.sample`, `.sh`, `.yaml` and `.yml`. Six names are exempt, and
...
- Do not deny `.md`. On Maho 26.9 and later, a page URL with a `.md` suffix is the
  [markdown version](../user-guide/ai-and-crawlers.md#markdown-for-ai-agents) that PHP generates
  for AI agents. A configuration written for an earlier version denies `.md`: remove it from the
  list, or every `.md` URL answers `404` before the request reaches PHP.
...
server {
    ...
    # .md is not listed: a page URL with a .md suffix is the markdown version
    # that PHP generates for AI agents.
    location ~* \.(dist|flag|ip|lock|neon|sample|sh|yaml|yml)$ {
        deny all;
    }
...
maho.example.com {
    ...
    route {
        ...
        # *.md is not listed: a page URL with a .md suffix is the markdown
        # version that PHP generates for AI agents.
        @private {
...
Also on 26.9 and later, a category URL with a `.md` suffix must answer `200` with
`Content-Type: text/markdown`:

```shell
curl -sI https://maho.example.com/women.md | grep -iE '^(HTTP|content-type)'
```

A `403` or `404` here means the server still denies `.md` files. Remove `md` from the list of
denied extensions. A `301` means the store adds or removes a trailing slash: repeat the check with
